...
id, a parse_config, an enabled predicate) and that any Integration contributing
whitelisted commands is first-party - its command builder lives under
``stormpulse/``. CORE-005 decision 8: registration is loader-ready, but only
in-tree code may touch the command surface until a future loader ADR grants a
trust boundary. This is the runtime sibling of Fn4's static dependency fence.
"""
