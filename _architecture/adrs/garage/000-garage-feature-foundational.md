...
| Substrate | Filesystem-agnostic at the agent layer. The 002-garage playbook commits to a substrate (currently ext4 with `metadata_fsync = true` + auto-snapshots); the agent does not enforce. |
...
- Multi-tenancy mapping. A control-plane concern, not the agent's.
...
| ZFS substrate precondition | removed (durability moved to `garage.toml` fsync + snapshots) |
