...
def parse_volume_mounts(compose_path: Path, project_dir: Path) -> list[Path]:
...
def run_system_setup(
    ...
    print(f"  Setting ownership on {project_dir}...", file=sys.stderr)
    if not _run_cmd(
        ["/usr/bin/chown", "-R", "root:stormpulse", str(project_dir)],
        description=f"chown -R root:stormpulse {project_dir}",
    ):
        print("    Cannot set project directory permissions. Do this manually.", file=sys.stderr)
        return
    ...
    # Restore ownership on Docker volume directories
    for vol_dir in volume_dirs:
        if vol_dir.is_dir():
            print(f"  Restoring Docker volume: {vol_dir}", file=sys.stderr)
            _run_cmd(
                ["/usr/bin/chown", "-R", "root:root", str(vol_dir)],
                description=f"chown -R root:root {vol_dir}",
            )
            _run_cmd(
                ["/usr/bin/chmod", "-R", "g-w", str(vol_dir)],
                description=f"chmod -R g-w {vol_dir}",
            )

    if volume_dirs:
        existing = [d for d in volume_dirs if d.is_dir()]
        if existing:
            print(f"  Excluded {len(existing)} Docker volume(s) from stormpulse ownership.", file=sys.stderr)
