...
def make_rotate_customer_key_handler(
    ...
    required = (
        "old_key_id", "new_key_name", "bucket_id", "local_alias", "key_tier",
    )
    ...
    if key_tier not in _VALID_TIERS:
        logger.error(
            "garage_rotate_customer_key got invalid key_tier: %r", key_tier,
        )
        return None
...
async def run_rotate_customer_key(
    ...
    state = _RotateState(
        bucket_id=bucket_id, local_alias=local_alias, perm_flags=perm_flags,
    )
    ...
    return JobOutcome(
        ...
        exit_code=0,
        stdout=(
            f"New key ID: {state.new_key_id}\n"
            f"Old key {old_key_id} deleted."
        ),
        extras={
...
async def _rollback(
    ...
    if state.new_key_id is not None:
        try:
            ...
            logger.warning(
                "Rollback: key delete %s failed: %s", state.new_key_id, exc,
            )
            manual.append({"type": "key", "id": state.new_key_id})
            return _RollbackResult(status="partial", manual_cleanup=manual)
        if rc != 0:
