...
class TestCmdUpdateFlagRejection:
    def test_branch_with_pip_source_rejects(
    ...
    def test_version_with_git_source_rejects(
        self, caplog: pytest.LogCaptureFixture
    ) -> None:
        with pytest.raises(SystemExit) as exc:
...
class TestCmdUpdatePipxMissing:
    def test_errors_when_pipx_not_on_path(
        self, caplog: pytest.LogCaptureFixture
    ) -> None:
...
class TestCmdUpdateRestartFlow:
    ...
    def test_system_mode_skips_auto_restart(
    ...
    def test_no_restart_skips_systemctl(
        self, capsys: pytest.CaptureFixture[str]
    ) -> None:
        calls: list[list[str]] = []
