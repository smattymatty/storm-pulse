"""Wire-tier harness: a real Garage, the real admin API, the real S3 endpoint.

Mocks nothing. This tier exists to answer one question that no fake can:
**does the agent still work against the Garage build the fleet actually runs?**
A Garage upgrade that renames a JSON field, changes a status code, or alters
an error string breaks the agent silently everywhere else in the suite. It
breaks here loudly, before a deploy.
...
Run it::
    ...
    make garage-up && make test-wire

Every test here carries the ``garage`` marker (applied automatically by
``pytest_collection_modifyitems`` below, so an author cannot forget it), and
``pyproject.toml`` deselects that marker by default. ``make check`` stays
Docker-free.

**Hard-fail, never skip.** If you asked for this tier and the container is
down, that is an error with the command to fix it. The old
``tests/garage/test_s3_integration.py`` skipped on four unset env vars, which
is why it never once ran.
"""
...
_UP_HINT = (
    ...
    "(or: docker compose -f docker/docker-compose.test.yml up -d)"
)


def pytest_collection_modifyitems(items: list[pytest.Item]) -> None:
    """Mark every test under ``tests/wire/`` as ``garage``.

    The marker is what ``pyproject.toml``'s default ``-m "not garage"``
    deselects. Applying it by directory rather than by hand means a new wire
    test cannot accidentally join the default suite and start demanding Docker
    of everyone running ``make check``.
    """
    for item in items:
        if "tests/wire/" in item.nodeid or item.nodeid.startswith("tests/wire"):
            item.add_marker(pytest.mark.garage)
