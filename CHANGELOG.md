...
- **`stormpulse init` offers to create a missing project directory.** Previously the wizard re-prompted with "Directory not found" and the operator had to drop out, `mkdir` in another shell, and start the wizard over. Now the wizard checks the deepest existing ancestor: if it's writable by the current user, it prompts "Create it? [Y/n]" and on yes calls `mkdir -p` and reports the resulting owner. If the ancestor isn't writable (e.g. `/opt` owned by root), the wizard skips the no-op "Create it?" prompt entirely, prints the exact `sudo mkdir -p ... && sudo chown $USER:$USER ...` line to run in another shell, and then prompts "Press Enter once `<path>` exists, or type a different path" — pressing Enter retries the same path so the operator keeps their answer. No subprocess `sudo`, no escalation surface.
