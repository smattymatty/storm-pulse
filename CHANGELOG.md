...
- **An Integration contributes its deploy subject, and the node still overrides it** (ADR CORE-009 decision 10). A node running an Integration should not be hand-told a fact it already knows about itself: enabling the integration yields its subject with nothing typed. The descriptor supplies a default and the node's own `[investigate.deploy.<subject>]` table overrides it field by field, including `enabled = false` to switch one off. Contributed subjects are deliberately NOT authoritative: an authoritative one would let a package update widen a search root on every node carrying that integration with no local brake. The subject still resolves from something installed on the box, never from the wire. New public SDK type `SdkDeploySubject`; no allow rule binds to a subject, so `command_specs_digest` does not move and no release order is forced.

- **`stormpulse investigate deploy --init` derives a subject from a unit file** (ADR CORE-009 decision 11), for boxes with no Integration. It offers the box's operator-installed units and reads `WorkingDirectory`, `ExecStart` and `FragmentPath` to propose the expected root, search roots and subject name; you confirm or edit. A unit file *is* the declaration of where its thing lives, which is what makes an out-of-root binary a finding by the node's own account rather than against a path someone remembered. Derivation refuses rather than guesses: a unit naming no `WorkingDirectory` and no absolute `ExecStart` yields no subject, and `/` is never an expected root, because an invented root gives a probe that reports cleanly about a place nothing was installed and on screen that is indistinguishable from health. The investigation itself stays one-shot and non-interactive; authoring config is a separate act.
...
- **A node may expose the deploy investigation over a road it bounds itself** (ADR CORE-009 decision 12). An operator may place a dedicated key in a node's `authorized_keys` behind `command=`, pointing at a runner that accepts only `investigate`. The bound lives on the node, not in the caller: an allowlist in a caller is a promise the caller makes about itself, and a bypassed or buggy caller keeps whatever access its key has, while a forced command is enforced by the subject and revoked by deleting one line. The runner accepts what the installed CLI has rather than a list it carries, so a new investigation is reachable as soon as the agent carrying it is installed, with no runner change. No caller-supplied window, pattern or config path, and no run-everything mode. This amends the first half of CORE-009's original refusal of an SSH probe; the second half stands, because that road runs the existing instrument rather than building a second one.

### Fixed

- **The golden log fixtures published in every wheel now carry synthetic values.** `wire_shape.py` and the Garage test fixtures stated a policy of using real captured output, and the captured lines carried a real client IP, a real Garage access key id and a real bucket alias. The SHAPE is still taken from real output, because an invented shape can pass a parser while missing the branch a real line takes; every VALUE is now a documentation address (RFC 5737), an obviously-fake key id and a placeholder bucket. A golden fixture ships to everyone who installs the package, so a captured value in one is a disclosure, not a test detail.
...
- **A security, secrets, dependency-CVE, and AI-defect gate now runs in CI and `make check`** (`make security`, Skylos). The exit code comes from a zero-tolerance `[tool.skylos.gate]` policy in pyproject.toml (no criticals, no highs, no secrets, no AI defects). Path-taint families (SKY-D215/D324/D325) are excluded with their reason documented in `[tool.skylos]`: Pulse's job is reading and writing operator-owned files, so an "attacker-controlled path" is the agent's normal input. Three false-positive CRITICALs carry inline reasoned ignores (an `ALTER TABLE` built from a hardcoded list, and two "SSRF"s that are the agent contacting its operator-configured endpoints). The config template's `hmac_secret` line moved out of the template string so the path-not-a-secret suppression lives in code, with byte-identical generated output.

- **A dead-code gate now runs in CI and `make check`** (`make deadcode`, Skylos 4.33.2, pinned in the dev extra). Scoped to unused functions, imports, variables, classes, and files (SKY-U001..U005); unused parameters (SKY-U006) stay out because callback signatures are interface conformance, not dead code. The tree starts clean: four dead symbols removed, one unused unpack renamed, and two deliberate contract declarations (`AuthError.REASONS`, `LongRunningFactory`) carry inline `# skylos: ignore` comments with their reasons.

- **Enrollment now carries the dashboard's explicit WebSocket transport URL.** New dashboards can keep ordinary-HTTPS enrollment on one hostname and return a separate mTLS-only `dashboard_url`; `stormpulse enroll` stores it in `enroll.json`, and `stormpulse init` prefers it as the generated config default. Older dashboards remain compatible: when the response omits the additive field, init retains the historical same-host derivation.

- **The file-browser walk is proven against a real Garage at scale** (`tests/wire/garage/test_walk_bucket_stats.py`, 11 tests). `walk_bucket_stats` backs every folder click in the dashboard's file browser, and its two hardest properties are invisible to a fake. **Pagination past 1000 objects**: ListObjectsV2 caps a page at 1000, so every real customer bucket is multi-page, and page two rides Garage's own continuation token back as a signed query parameter. The token this cluster returns literally begins with `]` (`]b2JqLTEwMDAuYmlu`), a character that must be percent-encoded in the SigV4 canonical query or the signature fails, so a broken re-sign would pass every fake test while silently truncating every large bucket's count at 1000. A 2050-object bucket (three-plus pages, seeded once and shared read-only) walks to an exact count and exact byte total, and the max_objects cap is checked at the page boundary. **Prefix encoding**: folder names with a space (`2024 vacation`), a plus (`a+b`), or a percent (`100%`) walk correctly, the recursive no-delimiter semantics count nested objects toward an ancestor folder, and a bare prefix matches literally (the reason the browser sends a slash-terminated prefix). The auth pre-flight's clean `auth_failed` classification (what the browser shows instead of a raw 403) is pinned too. Tests-only: no production change.
...
- **Unsealing a running agent no longer fires the whitelist-violation alarm on the operator's own verify block.** The seal has two enforcement layers and they are asymmetric by construction: `build_registry` excludes `run_verify_block` / `run_apply_block` when sealed (admission, built once at boot), and the dispatch recheck refuses them live (refusal only). Sealing mid-run is live because the recheck can refuse a command the boot registry still lists. Unsealing is not: the recheck can only refuse, never re-admit, and the registry is not rebuilt by an unseal or a reconnect, so an agent that booted sealed keeps the hatch commands out of its registry until it restarts. In that window a dispatched verify block fell through to the unknown-command path and emitted a `whitelist_rejection` security event with a bare `Unknown command: 'run_verify_block'`, the same signal a genuinely off-whitelist command raises. That cost a long staging hunt: the error named the wrong cause. `needs_restart_to_load` now detects the exact window (unsealed, but a hatch command is missing from the registry) on both the single-command and sequence paths and returns `failure_reason="restart_required"` with an actionable message, so the operator is told to restart the agent and the whitelist-violation signal stays clean. The `signoff unseal` CLI now says the same instead of claiming dispatch is re-enabled, which was false on a running agent, and `signoff status` names the restart when an agent may have booted sealed. ADR CORE-004 decision 6 is amended to document the asymmetry as deliberate defense-in-depth (while sealed the RCE command is not merely refused, it is not loaded at all).
